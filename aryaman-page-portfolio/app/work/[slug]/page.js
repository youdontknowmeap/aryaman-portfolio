...
export default function ProjectPage({ params }) {
  ...
  const ImagePlaceholder = ({ label, background, aspectRatio = "16/9" }) => (
    <div 
      className="w-full bg-[#111111] rounded-[4px] flex items-center justify-center overflow-hidden"
      style={{ aspectRatio, background }}
    >
      <span className="font-['Space_Mono'] text-[11px] text-[#707070] uppercase tracking-widest">
        {label}
      </span>
    </div>
  );
    ...
    if (project.slug === "drunkin") {
      return (
        <div className="space-y-24">
          ...
          {/* TODO: Replace with actual mockup image or video */}
  ...
  return (
    <div className="relative min-h-screen bg-[#080808]">
      ...
      <CaseNav currentSlug={project.slug} projects={projects} />
    </div>
  );
}
